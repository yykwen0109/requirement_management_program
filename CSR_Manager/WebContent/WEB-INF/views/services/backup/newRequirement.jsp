...
<body
	...
	<div class="app-body">
		...
		<ol class="breadcrumb">
			<li class="breadcrumb-item active">유지보수 사항</li>
			<!-- <li class="breadcrumb-item"><a href="allProjectsEnded">신규 프로젝트</a></li> -->
							...
							<div class="col-lg-12">
								<div class="card">
									<div class="card-header">
										<i class="fa fa-align-justify"></i> 유지보수 요구사항
									</div>
									<div class="card-body" style="overflow-x: overlay;">


										<c:forEach items="${list}" var="vo">
											<form class="form-horizontal" action="assignMember"
												method="post" id="form_${vo.request_Key}"></form>
										</c:forEach>
										...
										<table
											...
											<thead>
												<tr>
													<th colspan=2 style='text-align: center;'>배정</th>
													<th>프로젝트 이름</th>
													<th>요구사항 요청자 이름</th>
													<th>요구사항 등록일자</th>
													<th>요구사항 긴급도</th>
													...
													<th>확정</th>
													<th>비고</th>
													<th style="text-align: center; min-width: 60px">삭제</th>
												</tr>
											</thead>
											<tbody>
												...
												<c:forEach items="${list}" var="vo">
													<tr bgcolor="white">
														<td style="min-width: 100px;"><select
															class="form-control" style='font-size: 0.7em;'
															name="employee_key" form="form_${vo.request_Key}">
																<option value="0">담당자</option>
																<c:forEach items="${employee_list}" var="el">
																	<option value="${el.employee_key}">${el.employee_name}</option>
																</c:forEach>
														</select>
														</td>
														
														<td style="min-width: 60px">
														<input type="hidden" name="request_key" value="${vo.request_Key}" form="form_${vo.request_Key}">
															<button class="btn btn-sm btn-primary" type="submit" form="form_${vo.request_Key}">
																<i class="fa fa-dot-circle-o"></i> 배정
															</button>
														</td>
														<td>
															<c:forEach items="${list_project}" var="e">
														...
														<td>${vo.request_Emergency_Level}</td>
														<td>${vo.request_State}</td>
														<td>
															<c:forEach items="${enum_classification}" var="e">
															<c:if test="${vo.request_Classification == e.classification_Key}">
																${e.classification}
															</c:if>
															</c:forEach>
														${vo.request_Classification}
														</td>
														<td>${vo.request_Work_Classification}</td>
														<td>${vo.request_Content}</td>
														<td>${vo.request_Content_Detail}</td>
														<td>${vo.request_Content_Detail_Review}</td>
														<td>${vo.request_Confirmation}</td>
														<td>${vo.request_Remarks}</td>
														<td><input type="button"
															class="btn btn-sm btn-danger"
															onClick="location.href='${pageContext.request.contextPath}/deleteMember/${vo.request_Key}'"
															value="삭제" /></td>
													</tr>
												</c:forEach>
											</tbody>
