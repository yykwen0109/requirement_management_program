...
<body
	...
	<div class="app-body">
		...
		<ol class="breadcrumb">
			<li class="breadcrumb-item active">프로젝트 신청현황</li>
							...
							<div class="col-lg-12">
								<div class="card">
									<div class="card-header">
										<i class="fa fa-align-justify"></i> 프로젝트 신청현황
									</div>
											...
											<tbody>
												<c:forEach items="${newlist}" var="vo">
													<tr bgcolor="white">
														...
														<td>
															<c:forEach items="${enum_state}" var="e">
															<c:if test="${vo.project_State == e.state_key}">
																${e.state_name}
															</c:if>
															</c:forEach>
														</td>
														...
														<td>
															<c:forEach items="${enum_type}" var="e">
															<c:if test="${vo.project_Type == e.type_key}">
																${e.type_name}
															</c:if>
															</c:forEach>
														</td>
														<td>
															<c:forEach items="${enum_scope}" var="e">
															<c:if test="${vo.project_Scope == e.scope_key}">
																${e.scope_name}
															</c:if>
															</c:forEach>
														</td>
														...
														<td>
															...
															<c:if test="${vo.project_PM == e.employee_key}">
																${e.employee_name}
															</c:if>
															</c:forEach>
														</td>
														<td>
															<c:forEach items="${userlist}" var="e">
															<c:if test="${vo.project_Manager == e.employee_key}">
																${e.employee_name}
															</c:if>
															</c:forEach>
														</td>
